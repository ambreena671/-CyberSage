...
# Prioritized list of target aliases (exact matches prioritized over substring matches)
ALIASES = {
    'timestamp': [
...
def parse_logs(file_or_path):
    ...
    try:
        ...
        col_map_dict = dict(zip(clean_cols, df.columns))
        df.columns = clean_cols

        mapped_cols = {}

        # 2. Dynamic Prioritized Mapping Engine
        for target, keywords in ALIASES.items():
            # Pass A: Check for Exact Match First
            for kw in keywords:
                if kw in df.columns:
        ...
        if 'event' in mapped_cols:
            ...
            # Look for any text/object columns to construct meaningful event logs
            string_cols = df.select_dtypes(include=['object']).columns.tolist()
            if string_cols:
                df['event'] = df[string_cols].astype(str).agg(' | '.join, axis=1)
                warnings.append("event constructed from string features")
            else:
                df['event'] = "Suspicious Network Activity"
